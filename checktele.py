...
@sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.أبحث (.*)"))
async def _(event):
    if ispay2[0] == "yes":
        isclaim.clear()
        isclaim.append("on")
        ...
        @sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة أبحث"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
                    await event.edit(f"البحث وصل ({trys}) من المحاولات")
                elif "off" in isclaim:
